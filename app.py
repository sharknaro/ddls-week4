...
def structure(source: str):
    ...
    return Response((ROOT / item["path"]).read_text(encoding="utf-8"), media_type="text/plain")
...
function drawChart(){const c=$('chart'),d=devicePixelRatio||1,w=c.clientWidth,h=230;c.width=w*d;c.height=h*d;const x=c.getContext('2d');x.scale(d,d);const af=[];fetch('/api/structure/alphafold').then(z=>z.text()).then(t=>{const lines=t.split('\n');for(const l of lines){if(l.startsWith('ATOM')){const n=+l.slice(22,26),b=+l.slice(60,66);if(!af[n-1])af[n-1]=b}};let vals=af.filter(Boolean),sx=w/392;x.strokeStyle='#cbd5e1';for(let y of [50,70,90]){let py=h-y*h/100;x.beginPath();x.moveTo(0,py);x.lineTo(w,py);x.stroke()}x.strokeStyle='#334155';x.beginPath();vals.forEach((v,i)=>{let px=i*sx,py=h-v*h/100;i?x.lineTo(px,py):x.moveTo(px,py)});x.stroke();for(const [a,b,col] of [[18,28,'#f59e0b'],[94,312,'#2563eb'],[294,312,'#dc2626']]){x.fillStyle=col+'22';x.fillRect((a-1)*sx,0,(b-a+1)*sx,h)}x.fillStyle='#334155';x.font='12px sans-serif';x.fillText('pLDDT',5,15)})}
async function draw3d(){const status=$('viewer-status');try{if(!window.$3Dmol)throw new Error('3Dmol.js did not load');viewer=$3Dmol.createViewer($('viewer'),{backgroundColor:'white'});const [a,c]=await Promise.all([fetch('/api/structure/alphafold').then(x=>{if(!x.ok)throw Error('AlphaFold structure unavailable');return x.text()}),fetch('/api/structure/course').then(x=>{if(!x.ok)throw Error('Course structure unavailable');return x.text()})]);let ma=viewer.addModel(a,'cif'),mc=viewer.addModel(c,'pdb');if(!ma||!mc)throw Error('Structure format could not be parsed');ma.setStyle({}, {cartoon:{colorscheme:{prop:'b',map:color},opacity:.82}});mc.setStyle({}, {cartoon:{colorscheme:{prop:'b',map:color},opacity:.45}});ma.addStyle({resi:'18-28'},{stick:{color:'orange',radius:.18}});ma.addStyle({resi:'94-312'},{cartoon:{colorscheme:{prop:'b',map:color},opacity:.82}});ma.addStyle({resi:'294-312'},{stick:{color:'red',radius:.22}});mc.addStyle({resi:'294-312'},{stick:{color:'blue',radius:.15}});viewer.zoomTo();viewer.render();status.remove()}catch(err){status.textContent='3D structure could not be loaded: '+err.message;status.className='absolute inset-0 grid place-items-center text-sm text-red-700 p-4 text-center'}}init();</script></body></html>'''
