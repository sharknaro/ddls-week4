...
async function draw3d(){const status=$('viewer-status');try{if(!window.$3Dmol)throw new Error('3Dmol.js did not load');viewer=$3Dmol.createViewer($('viewer'),{backgroundColor:'white',antialias:true});const a=await fetch('/api/structure/alphafold').then(x=>{if(!x.ok)throw Error('AlphaFold structure unavailable');return x.text()});alphaModel=viewer.addModel(a,'cif');if(!alphaModel)throw Error('CIF structure could not be parsed');viewer.setStyle({}, {cartoon:{color:'#9ca3af',opacity:.9}});viewer.zoomTo({});viewer.render();console.info('3D viewer: grey cartoon rendered');status.remove()}catch(err){console.error('3D viewer failed:',err);status.textContent='3D structure could not be loaded: '+err.message;status.className='absolute inset-0 grid place-items-center text-sm text-red-700 p-4 text-center'}}init();</script></body></html>'''
